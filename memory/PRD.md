...
- **P0 (done, 2026-02-16)**: v1.2.0-alpha2 **Phase J — Factory Self-Evaluation Engine (IMPLEMENTED in OBSERVE MODE)**. Factory-level peer of Phase I Meta-Learning.
  - **Package**: `engines/factory_eval/` with `types`, `config`, `ledger`, `explainability`, `collectors` (7 aggregators), `evaluators` (10 pure functions), `engine` (proposers + ranker + applier + top-level cycle). Consolidated file layout keeps the code lean while preserving architectural clarity.
  - **4 operating modes** — default `observe`. Autonomous requires `FACTORY_EVAL_AUTONOMOUS_CONFIRM=YES` + per-surface whitelist. Q2 whitelist defaults to `compute_reallocation,execution_path_pref` only.
  - **10 evaluators**: `factory_improvement` (period-over-period KPI comparison), `provider_efficiency` (cost/pass per provider), `research_roi` (grouped pass-rate + cost), `strategy_ranking` (top-30/bottom-30 realised PnL), `regime_effectiveness` (per-regime Sharpe/hit-rate/delta), `bottleneck_detector` (top-3 summary per Q6), `compute_allocation` (diagnostic hint), `execution_quality_ranking` (path ranking), `portfolio_health_trends` (per-Master-Bot composite), `coverage_gap_detector` (unexplored cells).
  - **6 proposers**: `compute_reallocation`, `execution_path_pref`, `strategy_pruning` (flag-only per Q10), `portfolio_rebalance_hint` (hint-only), `research_investment` (hint-only), `budget_reallocation` (proportional-only, conservative gate).
  - **Ranker**: `score = expected_uplift × confidence × (1 − risk_penalty)`. Below `FACTORY_EVAL_RANK_FLOOR=0.01` → EXPIRED.
  - **Applier**: DORMANT in OBSERVE. Writes only to `factory_eval_overrides`; downstream engines opt in via `ORCH_USE_FACTORY_EVAL_OVERRIDES / EXEC_USE_FACTORY_EVAL_OVERRIDES / LEARNING_USE_FACTORY_EVAL_OVERRIDES` (all default `false`).
  - **Structural non-modification guarantee**: pytest hashes 11 watched files across brain/portfolio/execution/meta_learning; changes fail the suite.
  - **26 new API endpoints** under `/api/factory-eval/*`: config, status, reports, reports/latest, reports/{id}, kpis, insights, insights/{id}, recommendations, pending, recommendations/{id}, applications, overrides, mode-history, providers/leaderboard, strategies/top-contributors, strategies/pruning-candidates, portfolios/health-trends, execution/path-rankings, regimes/effectiveness, bottlenecks, coverage-gaps, refresh (admin), daily-report (admin), approve (admin — **409 in OBSERVE**), reject (admin), overrides/{target}/revert (admin).
  - **6 new Mongo collections**: `factory_eval_{reports (180d TTL), insights (90d TTL), recommendations (180d TTL via expires_at), applications (365d TTL — dormant in OBSERVE), overrides (permanent — dormant in OBSERVE), mode_history (365d TTL)}`. Idempotent index bootstrap.
  - **8 new outcome_event decision-type markers**: `factory_eval_cycle_start`, `factory_report`, `factory_eval_insight`, `factory_eval_recommendation`, `factory_eval_cycle_end`, `factory_eval_mode_change`, `factory_eval_application`, `factory_eval_revert`. Full immutable-ID audit chain.
  - **Orchestrator task**: `factory_evaluation` (priority 45, cadence 3600s, depends on `execution_attribution` + `meta_learning_evaluation`, respects `FACTORY_EVAL_MODE=disabled`). Task count 16 → **17**. Daily 90-day report at UTC `FACTORY_EVAL_DAILY_REPORT_HOUR=3`.
  - **59 Phase J tests + full A–J regression = 394 passing** (32 A + 22 B + 21 B.1 + 29 B.2 + 24 C + 33 D + 11 E + 25 F + 38 G + 45 H + 48 I + 59 J; running with `-n 0`).
  - **Router count 99 → 100** exactly. **Orchestrator task count 16 → 17**.
