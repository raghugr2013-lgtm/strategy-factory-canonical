/*
 * evaluationStore — Phase D1: 24-criterion checklist data + derived helpers.
 *
 * Ported from prototype/src/workspace-state/evaluationStore.ts. This D1
 * commit lands the DATA MODEL and the read-side derivations only. The
 * write-side actions (setVerdict / clearAll / markAllPass / setNotes /
 * setSession) land in D2 alongside the interactive controls in the
 * surface. Keeping the store already exposing the mutators means D2
 * lights up interactions with zero refactor.
 *
 * PROTOTYPE-INSPIRED, PRODUCTION-SAFE:
 *   - No new backend calls (this is a client-only walkable checklist).
 *   - Local persistence via localStorage (namespaced sf.eval.v1).
 *   - Compatible with the Sprint-1 formal Playwright + axe pipelines
 *     that eventually supersede this surface.
 */
import { create } from 'zustand';

// ─── Dimension catalogue (24 criteria across 6 dimensions) ─────────────
export const DIMENSIONS = [
  ...
  },
  {
    ...
    criteria: [
      { id: 'pred-1', headline: 'Predictable Return honoured Timeline → Passport → back.',
        detail: 'Back button copy reads "back to timeline"; row + facet are restored.',
        reference: 'E5 §4.5' },
      { id: 'pred-2', headline: 'Predictable Return honoured Approvals → Passport → back.',
      ...
      { id: 'pred-3', headline: 'Facet Bar cascade is visible across surfaces.',
        detail: 'Timeline actor, Approvals risk, Explorer status all persist between surfaces.',
...
const readPersisted = () => {
  try {
    ...
    return raw ? JSON.parse(raw) : { verdicts: {}, notes: '', session: '' };
  } catch { return { verdicts: {}, notes: '', session: '' }; }
};

const writePersisted = (p) => {
...
export const useEvaluationStore = create((set, get) => {
  ...
  return {
    ...
    // Mutators — declared now so D2 can enable UI without a refactor.
    setVerdict: (criterionId, v) => {
      const next = { ...get().verdicts, [criterionId]: v };
      writePersisted({ verdicts: next, notes: get().notes, session: get().session });
      set({ verdicts: next });
    },
    clearAll: () => {
      writePersisted({ verdicts: {}, notes: get().notes, session: get().session });
      set({ verdicts: {} });
    },
    markAllPass: () => {
      const next = {};
    ...
    setNotes: (notes) => {
      writePersisted({ verdicts: get().verdicts, notes, session: get().session });
      set({ notes });
    },
    setSession: (session) => {
      writePersisted({ verdicts: get().verdicts, notes: get().notes, session });
      set({ session });
    },
  };
});
...
// ─── Derived helpers ───────────────────────────────────────────────────
export const summariseDimension = (d, v) => {
  const s = { key: d.key, title: d.title, total: d.criteria.length,
...
export const overallReadiness = (v) => {
  ...
  if (fail > 0) return { verdict: 'blocked', pass, review, fail, unset, total, passPct,
    headline: 'Design Freeze BLOCKED.',
    detail: `${fail} criteri${fail === 1 ? 'on' : 'a'} failing. Author addenda and re-verify before requesting sign-off.` };
  if (pass === total) return { verdict: 'ready', pass, review, fail, unset, total, passPct,
    headline: 'Design Freeze READY.',
    detail: 'Every dimension is passing. Capture the summary and request operator sign-off.' };
  if (unset === total) return { verdict: 'unstarted', pass, review, fail, unset, total, passPct,
    headline: 'Evaluation not started.',
    detail: 'Walk each surface and mark verdicts against the six dimensions. Interactive verdict controls land in Phase D2.' };
