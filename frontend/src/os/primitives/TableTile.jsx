/*
 * TableTile — Bible §7.11.3 · dense sortable list.
...
const DENSITY_PAD = { compact: '6px 12px', cozy: '10px 14px', cinema: '14px 16px' };

export function TableTile({ caption, columns, rows, state = 'happy', onRowActivate, testId }) {
  ...
  const El = motionEnabled ? motion.section : 'section';
  ...
  return (
    <El data-testid={testId ?? `table-${code}`} {...motionProps}
      ...
      <div style={{ padding: 'var(--space-3) var(--space-4)', fontSize: 'var(--font-caption)',
                    color: 'var(--content-lo)', textTransform: 'uppercase', letterSpacing: '0.08em',
      ...
      {state === 'error' ? (
        <div style={{ padding: 'var(--space-5)' }}>
          <StateTemplate variant="error" code={`${code}-error`} icon={AlertTriangle} tone="crit"
                         headline="Table load failed." purpose="The evidence store returned an error." />
        </div>
      ) : state === 'empty' || (state === 'happy' && !rows.length) ? (
        <div style={{ padding: 'var(--space-5)' }}>
          <StateTemplate variant="empty" code={`${code}-empty`} icon={MinusCircle} tone="dormant"
                         headline="No rows match this filter." purpose="Clear a facet or widen the time window." />
        </div>
      ) : state === 'loading' ? (
        <div style={{ padding: 'var(--space-4)', display: 'flex', flexDirection: 'column', gap: 'var(--space-2)' }}>
          {[0,1,2,3,4].map((i) => (
            <div key={i} aria-hidden style={{ height: 14,
        ...
        <div role="table" aria-label={caption}>
          <div role="row" style={{ display: 'grid', gridTemplateColumns: gridCols,
            ...
            {columns.map((col) => (
              <button key={col.key} role="columnheader" data-testid={`${code}-col-${col.key}`}
          ...
          <div role="rowgroup">
            {sorted.map((row, ri) => (
              <div role="row" key={ri} tabIndex={0} data-testid={`${code}-row-${ri}`}
                ...
                {columns.map((col) => (
                  <div role="cell" key={col.key}
                       className={typeof row[col.key] === 'number' ? 'mono-num' : ''}
                       style={{ textAlign: col.align === 'right' ? 'right' : 'left',
                                fontSize: 'var(--font-body-sm)', color: 'var(--content-hi)',
