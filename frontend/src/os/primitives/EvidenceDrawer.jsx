...
export const EvidenceDrawer = ({
  open, onClose, title, subtitle, provenance, lineage,
  sections = [], state = 'happy', footerAction, testId,
}) => {
  ...
  return (
    <AnimatePresence>
      {open && (
        <>
          <motion.div data-testid={`${testId ?? 'evidence-drawer'}-overlay`}
                      initial="hidden" animate="visible" exit="hidden"
                      variants={motionEnabled ? fadeIn : { hidden: { opacity: 1 }, visible: { opacity: 1 } }}
                      style={{ position: 'fixed', inset: 0, background: 'rgba(5,7,10,0.6)',
                               backdropFilter: 'blur(4px)', zIndex: 40 }}
                      onClick={onClose} />
          <motion.aside data-testid={testId ?? 'evidence-drawer'}
                        ...
                        style={{ position: 'fixed', top: 0, right: 0, bottom: 0,
                                 width: 'min(560px, 90vw)', background: 'var(--surface-1)',
                                 borderLeft: '1px solid var(--stroke-2)', boxShadow: 'var(--elev-2)',
            ...
            <header style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between',
              ...
              <div>
                ...
                <div style={{ fontSize: 'var(--font-body-md)', color: 'var(--content-hi)', marginTop: 4 }}>{title}</div>
                {subtitle && (
                  <div className="mono-num" style={{ fontSize: 'var(--font-caption)',
                                                     color: 'var(--content-md)', marginTop: 4 }}>
                    {subtitle}
                  </div>
                )}
              </div>
              <button data-testid="evidence-drawer-close" aria-label="Close evidence drawer" onClick={onClose}
                      style={{ background: 'transparent', border: '1px solid var(--stroke-2)',
                               color: 'var(--content-md)', borderRadius: 'var(--radius-1)',
            ...
            <div style={{ overflowY: 'auto', padding: 'var(--space-4) var(--space-5)',
              ...
              {state === 'error' ? (
                <StateTemplate variant="error" code="evidence-error" icon={AlertTriangle} tone="crit"
                               headline="Evidence unavailable." purpose="The evidence store returned an error." />
              ) : state === 'empty' ? (
                <StateTemplate variant="empty" code="evidence-empty" icon={MinusCircle} tone="dormant"
                               headline="No sections attached."
                               purpose="This artefact has provenance but no attached notes yet." />
              ) : state === 'loading' ? (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-2)' }}>
                  {[0,1,2].map((i) => (
                    <div key={i} aria-hidden style={{ height: 40,
                ...
                sections.map((s) => (
                  <section key={s.heading}
                           data-testid={`evidence-section-${s.heading.replace(/\W+/g, '-').toLowerCase()}`}>
                    <div style={{ fontSize: 'var(--font-caption)', color: 'var(--content-lo)',
            ...
            {footerAction && (
              <footer style={{ padding: 'var(--space-3) var(--space-5)',
                ...
                <button data-testid={footerAction.testId ?? 'evidence-drawer-footer-action'}
