 ...
 * One hop up (ancestors) + one hop down (descendants) as a compact strip.
 * Sprint 1 does NOT include Graph mode (deferred, per D8).
 * Root generation and Replay-empty states use StateTemplate.
 */
...
export interface LineageNode {
  id: string;
  label: string;
  kind?: string;         // e.g. "candle", "signal", "backtest"
}

export interface LineageBarProps {
  self: LineageNode;
  ancestors?: LineageNode[];
  descendants?: LineageNode[];
  onOpen?: (n: LineageNode) => void;
  replayEmpty?: boolean;
  testId?: string;
}

const NodeChip: React.FC<{ n: LineageNode; onOpen?: (n: LineageNode) => void; muted?: boolean; strong?: boolean; }> =
  ({ n, onOpen, muted, strong }) => (
    <button
...
export const LineageBar: React.FC<LineageBarProps> = ({
  self, ancestors = [], descendants = [], onOpen, replayEmpty, testId,
}) => {
  if (replayEmpty) {
    return (
      <StateTemplate
        variant="replay-empty"
        code="lineage-replay-empty"
        icon={GitBranch}
        tone="dormant"
  ...
  if (!ancestors.length && !descendants.length) {
    return (
      <div
        data-testid={testId ?? 'lineage-bar-root'}
        style={{
  ...
  return (
    <div
      data-testid={testId ?? 'lineage-bar'}
      style={{
        display: 'flex', alignItems: 'center', gap: 'var(--space-2)',
        padding: 'var(--space-3)',
        background: 'var(--surface-1)',
